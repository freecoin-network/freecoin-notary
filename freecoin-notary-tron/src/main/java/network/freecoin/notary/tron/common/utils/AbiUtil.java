...
import network.freecoin.notary.tron.common.crypto.Hash;
import network.freecoin.notary.tron.exception.EncodingException;
...
public class AbiUtil {
  ...
  static class CoderArray extends Coder {
    ...
    byte[] encode(String arrayValues) throws EncodingException {
      ...
      if (this.length == -1) {
        return ByteUtil
            .merge(new DataWord(strings.size()).getData(),
                pack(coders, strings));
      } else {
  ...
  static class CoderToken extends Coder {
    ...
    @Override
    byte[] encode(String value) {
      String hex = Hex.toHexString(
          new DataWord(value.getBytes()).getData());
  ...
  public static byte[] pack(List<Coder> codes, List<Object> values) throws EncodingException {
    ...
    for (int idx = 0; idx < codes.size(); idx++) {
      ...
      if (coder.dynamic) {
        System.arraycopy(new DataWord(dynamicOffset).getData(),
            0, data, offset, 32);
        offset += 32;
...
//    String str = "1,[\\\"TNNqZuYhMfQvooC4kJwTsMJEQVU3vWGa5u\\\",\"TNNqZuYhMfQvooC4kJwTsMJEQVU3vWGa5u\"]";
    ...
    ;
    l.add(
        Arrays.asList("TNNqZuYhMfQvooC4kJwTsMJEQVU3vWGa5u", "TNNqZuYhMfQvooC4kJwTsMJEQVU3vWGa5u"));
...
//    int offset = DataWord.getDataWord(data, 0).intValue();
    ...
    for (int i = 0; i < length; i++) {
      DataWord dataWord = DataWord
          .getDataWord(data, 2 + i);
      if (!dataWord.isZero()) {
        byte[] noprefix = ByteUtil
            .stripLeadingZeroes(dataWord.getData());
        addressList.add(
            WalletUtil.encode58CheckWithoutPrefix(noprefix));
      }
    }
    return addressList;
  }
